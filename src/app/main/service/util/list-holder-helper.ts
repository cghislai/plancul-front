...
import {distinctUntilChanged, filter, map, mergeMap, publishReplay, refCount, switchMap, take, tap} from 'rxjs/operators';
...
export class ListHolderHelper<E extends WsDomainEntity, FILTER> {
  ...
  constructor(searchFunction: (filter: FILTER, pagination: Pagination) => Observable<WsSearchResult<WsRef<E>>>,
    ...
    const searchResults: Observable<WsSearchResult<WsRef<E>>> = combineLatest(this.filterSource, nonNullPagination)
      .pipe(
  ...
  reload() {
    this.filterSource.pipe(
      take(1),
    ).subscribe(curFilter => this.filterSource.next(curFilter));
  }
